#Below wrapper code was referred from https://github.com/richardsun-voyager/DeepLearningModelsforMNIST
import warnings
...
with tf.Session(graph=graph) as sess:
	...
	sess.run(init_op)
	#Train the datase
	fout = open('log_without_bn.txt', 'w')
	fout.close()
	for eeee in range(epochs):
		l, a = 0, 0
		fout = open('log_without_bn.txt', 'a')
		...
		fout.close()
		print('eeee:', str(eeee), 'Loss:{:.5f}'.format(l), 'Accuracy:{:.5f}'.format(a))
	...
	for _ in range(200):
		...
		#print(cp)
		count += np.sum(cp)
	print("Testing Accuracy：", count/n_ts)	# 0.9782
